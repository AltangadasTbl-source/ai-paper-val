...
- Checked UTC: 2026-08-19T04:39:46Z
- Provider/authentication inference: PASS — the current interactive coordinator produced a model response under the package-pinned fixed coordinator route; provider credentials remain outside the package configuration.
- Dynamic aliases: Not used.

## Verified Routing Matrix

| Named preset | Required model | Required effort | Configuration result |
|---|---|---|---|
...
The project coordinator configuration and all nine named preset files were inspected locally before scientific work. Every mandatory specialist stage will use a fresh runtime agent; the two statistical passes will use distinct fresh runtime IDs.
