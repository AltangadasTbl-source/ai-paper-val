#!/usr/bin/env python3
"""Validate workflow 1.3 coverage, uncapped candidate conservation, links, tone, and integrity."""
...
def main() -> None:
    ...
    parser.add_argument("--profile", required=True, choices=("1.3.1", "1.3.2"))
    ...
    if (run_dir / "review_queue.md").exists():
        errors.append("Workflow 1.3 must not create a count-limited review_queue.md.")
    ...
    if args.profile == "1.3.1":
        ...
        if any(relative.startswith(new_prefix) for relative in reused_hashes):
            errors.append("The reused-artifact inventory contains new workflow-1.3.1 outputs.")
    ...
    for candidate_id, section in report_sections.items():
        ...
        if not any(category_line.strip() == f"**Category:** {value}" for value in ALLOWED_CATEGORIES):
            errors.append(f"{candidate_id} does not use one exact workflow-1.3 category.")
