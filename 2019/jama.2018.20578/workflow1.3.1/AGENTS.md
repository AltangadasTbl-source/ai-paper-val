# Quantitative Quality-Control Review 1.3.1 — Reuse Existing Evidence Assets
...
candidates. Workflow 1.3.1 starts candidate discovery again without a count limit, while reusing all
...
- `.ai_paper_validation/review_1_3_1/`
...
extractor, report renderer, and final validator in `workflow_1_3_1/scripts/`.
...
1. Inventory every direct source and record SHA-256 in
   `.ai_paper_validation/review_1_3_1/source_hashes_before.sha256`.
2. Use `qc13_reuse_asset_curator` to inventory and hash every reused OCR/text/table/page/document-map
...
5. Run `qc13_numeric_consistency_reviewer`, `qc13_statistical_consistency_reviewer` pass 1, and
   `qc13_cross_source_consistency_reviewer` in parallel. Each lane examines all assigned relationships
...
7. Run `qc13_evidence_rechecker` over every stable ID against the exact source page or truthful Office
...
9. Run `qc13_quality_control_auditor` over every stable ID and every coverage row. Repair supportable
   omissions, but never delete or suppress an assigned candidate ID.
10. Run `qc13_report_generator`. The Markdown and HTML reports must contain every candidate in the
    ledger. If no candidate is found after complete coverage, produce a zero-candidate report that
    states the completed scope and limitations. For a large ledger, generate disjoint candidate-card
    parts in parallel waves and merge all parts before rendering; never shorten the report to fit one
    agent call.
11. Recompute source and reused-artifact hashes, then run:

```bash
python3 workflow_1_3_1/scripts/validate_review.py --profile 1.3.1 --package .
```

Repair correctable defects until
`.ai_paper_validation/review_1_3_1/review_validation.json` reports `PASS`.
