# Quantitative Quality-Control Review 1.3.2 — Full Source-First Restart

The current project root is exactly one paper package containing one main article and zero or more
support files. Workflow 1.3.2 restarts the entire review from supplied sources, including inventory,
...
- `.ai_paper_validation/review_1_3_2/`
...
extractor, report renderer, and final validator in `workflow_1_3_2/scripts/`.
...
- `workflow_1_3_2/review_contract.md`
- `workflow_1_3_2/report_spec.md`
- `workflow_1_3_2/settings.toml`
...
1. Inventory every direct source and record SHA-256 in
   `.ai_paper_validation/review_1_3_2/source_hashes_before.sha256`.
2. Use `qc13_fresh_source_preprocessor` to classify all direct sources and create a new evidence-asset
   inventory under `review_1_3_2/`. Extract PDF native text and layout text directly. Convert Office
...
4. Run `qc13_main_quantitative_mapper` and `qc13_support_quantitative_mapper` in parallel over disjoint
   newly prepared units. Map all result-relevant numeric and statistical relationships.
5. Run `qc13_numeric_consistency_reviewer`, `qc13_statistical_consistency_reviewer` pass 1, and
   `qc13_cross_source_consistency_reviewer` in parallel. Each lane completes every assigned
...
7. Run `qc13_evidence_rechecker` over every stable ID against exact source locations.
...
9. Run `qc13_quality_control_auditor` over every stable ID and coverage row. Repair supportable
   omissions, but never delete or suppress an assigned candidate ID.
10. Run `qc13_report_generator`. Markdown and HTML must contain every ledger candidate. If complete
    coverage finds none, produce a zero-candidate report documenting coverage and limitations. For a
    large ledger, generate disjoint candidate-card parts in parallel waves and merge all parts before
    rendering; never shorten the report to fit one agent call.
11. Recompute source hashes, then run:

```bash
python3 workflow_1_3_2/scripts/validate_review.py --profile 1.3.2 --package .
```

Repair correctable defects until
`.ai_paper_validation/review_1_3_2/review_validation.json` reports `PASS`.
