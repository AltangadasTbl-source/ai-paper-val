#!/usr/bin/env python3
"""Render a versioned legacy-patch report as standalone locally linked HTML5."""
...
def fallback_render(source: Path, destination: Path, title: str, css_path: Path) -> None:
    ...
    while index < len(lines):
        ...
        if line.startswith("```"):
            ...
            rendered.append(f'<h{level} id="{identifier}">{inline_markdown(heading)}</h{level}>')
            if level <= 3:
    ...
    notice = (
        ...
        "the legacy patch preserves and mechanically rechecks candidates but does not assign "
        "AI validity, severity, acceptance, rejection, or correction decisions.</p>"
    )
    document = (
...
def main() -> None:
    ...
    parser.add_argument(
        "--title", default="AI Paper Validation Recovery Report — Pending Human Adjudication"
    )
    ...
    if not pandoc:
        fallback_render(source, destination, args.title, css_path)
