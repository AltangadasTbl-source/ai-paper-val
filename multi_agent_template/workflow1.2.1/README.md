...
- 原始 1.0 报告不覆盖，新报告名为 `final_report_1_2_1.md/html`。
...
cp -a /absolute/path/to/workflow1.2.1/. <paper>/
```

这只会替换工作流管理文件；`.ai_paper_validation/` 和原始论文文件不会被复制命令触碰。
旧 `.codex/agents/` 中未同名的 1.0 agent 可能仍保留，但 1.2.1 的 `AGENTS.md` 只会调用本
补丁的唯一角色名。
...
如果旧 `.ai_paper_validation/final_report.md` 已经由
`final_report_endetail_prompt.md` 二次改写，请改用 workflow 1.2.2。预检脚本会自动识别并
停止错误版本，不能强行绕过。
...
├── final_report_1_2_1.md             # 新的人审报告，最多 10 个候选
