# Legacy AI Paper Validation Patch 1.2.1

This project root is exactly one paper package previously processed by workflow 1.0 **without** the
later `final_report_endetail_prompt.md` pass. This is a recovery run, not a fresh exhaustive audit.
...
- `.ai_paper_validation/patch_1_2_1/`
...
```bash
python3 legacy_patch/scripts/prepare_recovery.py --profile 1.2.1 --package .
```

The script must report `NOT_DETAILED`. If it reports that an endetail report is present, record that
workflow 1.2.2 is required and stop this profile without asking the user. Never bypass the profile
check.
...
- `.ai_paper_validation/patch_1_2_1/legacy_inventory.md`
...
Patch 1.2.1 separates these concepts. Preserve all distinct legacy candidate records in a recovery
...
1. Use `legacy_record_mapper` to inspect every candidate-bearing artifact listed by the preflight
   inventory. It writes complete source coverage, lineage, and an unbounded recovered ledger.
...
3. Reuse existing text/OCR/page images first. Reopen the exact original PDF pages for confirmation.
   ...
   `patch_1_2_1/targeted_preprocessing/`; a missing LibreOffice binary is non-fatal.
...
5. Build `.ai_paper_validation/patch_1_2_1/review_queue.md` with no more than 10 candidates, following
...
```bash
python3 legacy_patch/scripts/validate_patch.py --profile 1.2.1 --package .
```
...
`.ai_paper_validation/patch_1_2_1/patch_validation.json` reports `PASS`.
