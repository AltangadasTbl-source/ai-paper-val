...
def main() -> None:
    ...
    if args.profile == "1.2.2":
        legacy_report_ids = CANDIDATE_HEADING.findall(read(legacy / "final_report.md"))
        harvest = read(patch_dir / "endetail_harvest.md")
        for candidate_id in dict.fromkeys(legacy_report_ids):
            if not re.search(rf"\b{re.escape(candidate_id)}\b", harvest):
                errors.append(f"Endetail candidate is absent from endetail_harvest.md: {candidate_id}")
