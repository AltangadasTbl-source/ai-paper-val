Project path:
  ...
  Every PDF citation link must deep-link to the cited PDF page by appending a
  standard page fragment to the relative path: `#page=<PDF-page-number>`. A link
  that only opens the PDF at its first/default page is insufficient when a
  specific evidence page is cited. Examples:

  - Single page: `[paper.pdf](../paper.pdf#page=8), PDF p. 8`.
  - Page range: `[paper.pdf](../paper.pdf#page=53), PDF pp. 53-54`; link to the
    first page of the cited range.
  - Two noncontiguous pages: provide two separate links, each with its own page
    fragment.
  - Two sources: provide a separate page-targeted relative link for each source.

  Document-level inventory or hash links that do not cite a particular result
  must still use a relative link and should target PDF p. 1 with `#page=1`.
  ...
  that it remains clickable in both Markdown and rendered HTML. Do not use an
  absolute filesystem path, `file://` URL, drive-qualified path, web URL, or a
  link without a PDF page fragment. From `.ai_paper_validation/final_report.md`,
  source-PDF links normally have the form `../filename.pdf#page=N`.
  ...
  **Reasoning procedure**
  ...
  7. Preserve clickable relative source links, and require every PDF link to
     include the correct `#page=N` fragment for the cited PDF page.
  ...
  9. Every relative PDF/XLSX link resolves to an existing file; every PDF link
     is relative, contains the correct `#page=N` fragment, and opens at the cited
     PDF page in a compatible viewer. No absolute path appears in Markdown or
     HTML link targets.
