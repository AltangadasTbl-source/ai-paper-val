Project path:
  ...
  that it remains clickable in both Markdown and rendered HTML. Every PDF link
  must include a `#page=N` fragment pointing to the exact cited PDF page. A
  file-level inventory or hash link with no more specific citation must point to
  `#page=1`. When one statement cites multiple noncontiguous PDF pages, provide
  a separate page-targeted relative link for each cited page.

  Do not use absolute filesystem paths, `file://` URLs, drive-letter paths, or
  root-relative paths anywhere in `final_report.md` or `final_report.html`.
  Source links must remain relative to `.ai_paper_validation/`, for example
  `../source.pdf#page=8`. The rendered HTML must preserve the same relative
  path and page fragment. PDF page fragments are the deepest portable target;
  identify any table, figure, panel, row, column, footnote, or paragraph in the
  visible link text or the immediately adjacent citation text.
  ...
  **Reasoning procedure**
  ...
  7. Preserve clickable, page-targeted relative source links. Every PDF link
     must contain the correct `#page=N` fragment; no absolute path is allowed.
  ...
  10. Every PDF link in Markdown and HTML contains a valid `#page=N` fragment
      that matches the cited PDF page and falls within the source page count.
  11. Neither Markdown nor HTML contains an absolute local path, `file://` URL,
      drive-letter path, or root-relative source link.
  12. The HTML contains the revised findings and table of contents and preserves
      all relative paths and PDF page fragments.
