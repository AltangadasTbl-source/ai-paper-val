Project path:
  ...
  Instead, use the complete actual source filename everywhere. Every PDF link
  must use a relative path from `.ai_paper_validation/` and must include the
  exact PDF-page fragment `#page=N`, for example:

  [joi250046supp4_prod_1755300121.15587.pdf, PDF p. 8](../joi250046supp4_prod_1755300121.15587.pdf#page=8),
  eTable 3.

  The link must open the cited source PDF at the corresponding PDF page in PDF
  viewers that support standard page fragments. A link to the PDF file without
  `#page=N` is insufficient for a page-specific citation. If one statement cites
  several PDF pages, repeat the relative link for every cited page and give each
  link its own matching fragment; do not attach one link to a multi-page citation
  and leave the remaining pages unlinked. For a package-level file reference that
  is not tied to a specific evidentiary page, link to `#page=1`.

  Do not use absolute filesystem paths, Windows drive paths, `file://` URIs, or
  workspace-specific URLs in `final_report.md` or `final_report.html`. Link
  destinations must remain portable relative paths such as `../filename.pdf#page=8`.
  ...
  Make every filename a page-targeted relative Markdown link to the original
  source file so that it remains clickable in both Markdown and rendered HTML.
  The HTML renderer must preserve the relative `href` and its `#page=N` fragment.
  ...
  **Reasoning procedure**
  ...
  7. Preserve clickable, page-targeted relative source links. Every PDF link must
     include a valid `#page=N` fragment matching the cited PDF page, and no report
     link may use an absolute path.
  ...
  Run the command from `.ai_paper_validation/` so the relative source links and
  their `#page=N` fragments remain valid.
  ...
  9. Every PDF/XLSX link uses a relative path and resolves to an existing file;
     no absolute path or `file://` URI remains. After stripping each PDF fragment,
     verify the base file exists. Verify every PDF link includes `#page=N`, that
     `N` matches the cited PDF page, and that `1 <= N <=` the source PDF page count.
  10. The HTML contains the revised findings and table of contents, and preserves
      every page-targeted relative PDF `href`, including its `#page=N` fragment.
