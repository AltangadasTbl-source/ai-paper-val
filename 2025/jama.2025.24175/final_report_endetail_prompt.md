Project path:
  ...
  Make every filename a relative Markdown link to the original source file.
  Every PDF evidence link must include a `#page=N` fragment matching the cited
  PDF page so that selecting the link opens the corresponding source page, for
  example `../source.pdf#page=8`. Use a separate link for each separately cited
  PDF page. Do not use absolute filesystem paths, `file://` URLs, drive-qualified
  paths, or web URLs. The relative deep links must remain intact and clickable in
  both Markdown and rendered HTML.
  ...
  **Reasoning procedure**
  ...
  7. Preserve clickable relative source links and their page-level `#page=N`
     fragments; do not use absolute paths.
  ...
  9. Every relative PDF/XLSX link resolves to an existing file; every PDF
     evidence link contains the correct `#page=N` fragment, and neither Markdown
     nor HTML contains an absolute source-file path.
