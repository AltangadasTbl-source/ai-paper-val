# Quantitative Quality-Control Review 1.5.1 — Reuse Existing Evidence Assets
...
candidates. Workflow 1.5.1 starts candidate discovery again without a count limit, while reusing all
...
- `.ai_paper_validation/review_1_5_1/`
...
`workflow_1_5_1/scripts/`. The calculator summarizes authoritative runtime usage; it does not estimate
...
- `workflow_1_5_1/review_contract.md`
- `workflow_1_5_1/report_spec.md`
- `workflow_1_5_1/settings.toml`
...
1. Record the UTC start time, then inventory every direct source, its complete stable unit count, and
   SHA-256 in `.ai_paper_validation/review_1_5_1/source_hashes_before.sha256`. Set a package-specific
...
2. Use `qc15_reuse_asset_curator` to inventory and hash every reused OCR/text/table/page/document-map
...
5. Run `qc15_main_quantitative_mapper` and `qc15_support_quantitative_mapper` in parallel over the
...
6. Run numeric and cross-source review in new Terra/medium agents. Independently spawn a new
...
8. Run `qc15_evidence_rechecker` over every stable ID against the exact source page or truthful Office
...
9. Spawn a different new `gpt-5.6-terra`/`high` agent for statistical pass 2 over every statistical
   ...
   every appended ID.
10. Run `qc15_quality_control_auditor` over every stable ID, every coverage row, the complete
...
11. Run `qc15_report_generator` to assemble the complete Markdown report. It must contain every
...
12. Immediately after Markdown assembly, finalize `run_state.md`. Treat `Finished UTC` as the token-
    ...
    package total, and token-only price estimate into the report. Recompute source and reused-artifact
    hashes, render HTML once, and validate:

```bash
python3 workflow_1_5_1/scripts/calculate_token_cost.py \
  --ledger .ai_paper_validation/review_1_5_1/token_usage_ledger.csv \
  --pricing workflow_1_5_1/token_pricing.toml \
  --markdown .ai_paper_validation/review_1_5_1/token_usage_summary.md \
  --json .ai_paper_validation/review_1_5_1/token_usage_summary.json
python3 workflow_1_5_1/scripts/render_report.py \
  .ai_paper_validation/final_report_1_5_1.md \
  .ai_paper_validation/final_report_1_5_1.html \
  --profile 1.5.1
python3 workflow_1_5_1/scripts/validate_review.py --profile 1.5.1 --package .
```

Repair correctable defects until
`.ai_paper_validation/review_1_5_1/review_validation.json` reports `PASS`.
