Project path:
  ...
  [joi250046supp4_prod_1755300121.15587.pdf](../joi250046supp4_prod_1755300121.15587.pdf#page=8),
  PDF p. 8, eTable 3.
  ...
  Make every filename a relative Markdown link to the original source file and
  add a standard 1-based PDF page fragment, `#page=<PDF-page-number>`, so a click
  opens the cited page rather than only the beginning of the paper. For example:

  [joi250046supp4_prod_1755300121.15587.pdf](../joi250046supp4_prod_1755300121.15587.pdf#page=8),
  PDF p. 8, eTable 3.

  If one statement relies on more than one PDF page, provide a separate relative
  link for every cited page. Do not use one page anchor to represent a different
  page. Preserve the `#page=N` fragments in rendered HTML.

  All report links must be relative to `final_report.md`. Do not use absolute
  filesystem paths, Windows drive paths, `file://` URIs, or `http://`/`https://`
  links for supplied local sources. When `final_report.md` is stored under
  `.ai_paper_validation/` and the source PDF is in the project root, the expected
  form is `../actual_filename.pdf#page=N`.
  ...
  **Reasoning procedure**
  ...
  7. Preserve clickable relative source links. Every PDF source link must include
     the correct `#page=N` fragment and must open the exact cited PDF page.
  ...
  - Linked correctly to the original local files and exact cited PDF pages
  ...
  9. Every PDF/XLSX link is relative; no report link uses an absolute filesystem
     path, drive-qualified path, `file://` URI, or web URL for a local source.
  10. Every PDF link contains `#page=N`, where `N` is a valid 1-based PDF page
      matching the adjacent citation. Strip the fragment before checking that
      the relative PDF path resolves to an existing file.
  11. When a statement cites multiple PDF pages, each page has its own direct
      relative link; no link points only to the beginning of a cited paper.
  12. The HTML preserves every relative PDF `href` and `#page=N` fragment and
      contains the revised findings and table of contents.
  13. `final_report.html` is valid UTF-8 HTML.
  14. Source PDFs and workbooks remain unchanged.
  15. `git diff --check` reports no newly introduced formatting errors.
