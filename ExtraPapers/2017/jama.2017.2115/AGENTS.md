...
support files. Workflow 1.5.2 restarts the entire review from supplied sources, including inventory,
...
- `.ai_paper_validation/review_1_5_2/`
...
- `workflow_1_5_2/review_contract.md`
- `workflow_1_5_2/report_spec.md`
- `workflow_1_5_2/settings.toml`
...
1. Record the UTC start time, then inventory every direct source, its complete stable unit count, and
   SHA-256 in `.ai_paper_validation/review_1_5_2/source_hashes_before.sha256`. Set a package-specific
...
2. Use `qc15_fresh_source_preprocessor` to classify all direct sources and create a new evidence-asset
   inventory under `review_1_5_2/`. Extract PDF native text and layout text directly. Convert Office
...
4. Run `qc15_main_quantitative_mapper` and `qc15_support_quantitative_mapper` in parallel over disjoint
   newly prepared units. Map all result-relevant numeric and statistical relationships.
5. Run numeric and cross-source review in new Terra/medium agents. Independently spawn a new
...
7. Run `qc15_evidence_rechecker` over every stable ID against exact source locations.
8. Spawn a different new `gpt-5.6-terra`/`high` agent for statistical pass 2 over every statistical
   ...
   every appended ID.
9. Run `qc15_quality_control_auditor` over every stable ID, coverage row, source-coverage row, and the
   statistical execution manifest. Repair supportable
   omissions, but never delete or suppress an assigned candidate ID.
10. Run `qc15_report_generator` to assemble the complete Markdown report. It must contain every ledger
...
11. Immediately after Markdown assembly, finalize `run_state.md`. Treat `Finished UTC` as the token-
    ...
    package total, and token-only price estimate into the report. Recompute source hashes, render HTML
    once, and validate:
